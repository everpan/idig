...
func TestDM_Update(t *testing.T) {
	...
	engine.Exec("DELETE FROM idig_entity_attr_group WHERE entity_name = 'student' OR attr_table = 'student1'")
	...
	if err != nil {
		t.Fatalf("insert student0 err: %v", err)
	}
	s1.Idx = s0.Idx
	_, err = engine.Insert(s1)
	if err != nil {
		t.Fatalf("insert student1 err: %v", err)
	}
	...
	tests := []struct {
		...
		},
		{
			...
			check: func(body string, err error) {
				...
				if s0.Name != "updated_test2" || s0.Mobile != "13800138001" || s1.Gender != "2" {
					t.Errorf("update result not match")
				}
			},
		},
		{
			...
			check: func(body string, err error) {
				if err != nil {
					t.Errorf("update should return success even if no rows affected: %v", err)
				}
				// 验证没有更新任何数据
				var s Student0
				has, err := engine.Where("name = ?", "should_not_update").Get(&s)
				if err != nil || has {
		...
		},
		{
			...
			check: func(body string, err error) {
				...
				// 验证更新结果
				var s Student0
				has, err := engine.Where("mobile LIKE ? AND idx IN (SELECT idx FROM student1 WHERE class_id = ?)", "138%", 1).Get(&s)
				if err != nil || !has {
					t.Errorf("verify update failed: %v", err)
				}
...
func TestDM_Update2(t *testing.T) {
	tenant := config.DefaultTenant
	engine, _ := config.GetEngine(tenant.Driver, tenant.DataSource)
	...
	// 清理元数据
	engine.Exec("DELETE FROM idig_entity WHERE entity_name = 'student'")
	engine.Exec("DELETE FROM idig_entity_attr_group WHERE entity_name = 'student'")
	...
	tests := []struct {
		...
		{"no pk", `{"vals":{"name":"nam1","card":"c1","gender":"male"}}`, "student",
			func(body string, err error) {
				// sqlite unique null 的情况下 可以反复插入
				assert.Contains(t, body, "not implement")
			}},
		{"column is not exit", `{"vals":{"idx":3,"not-exit":"not exist","name":"nam1","card":"c22","gender":"female"}}`, "student",
			func(body string, err error) {
				assert.Contains(t, body, `"data":"column 'not-exit' not found"`)
			}},
		{"uk", `{"vals":{"idx":2,"mobile":"uk1-2","name":"nam1-2","card":"c1-2","gender":"male-2","class_id":"--2"}}`, "student",
			func(body string, err error) {
				var chk = strings.Contains(body, "insert 1 rows") || strings.Contains(body, "UNIQUE constraint failed")
				assert.True(t, chk)
			}},
...
"vals":[["uk321","nam1","c1","male",234],["uk322","nam1","c1","male",245]]}`, "student",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodPut, "/api/v1/entity/dm/"+tt.entity, bytes.NewReader([]byte(tt.req)))
			resp, err := app.Test(req, -1)
			// assert.Nil(t, err)
			body, err := io.ReadAll(resp.Body)
			t.Log(string(body))
